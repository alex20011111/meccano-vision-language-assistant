...
ap.add_argument("--sintetico", required=True,
                help='synthetic dataset folder containing classi.json and train/')
ap.add_argument("--output", default="dataset_yolo")
ap.add_argument("--target", type=int, default=2000,
                help='target total image count '
                     '(originals + augmented images)')
ap.add_argument("--val_frac", type=float, default=0.12,
                help='fraction of ORIGINAL images reserved for validation')
...
if not os.path.exists(path_classi):
    raise SystemExit(f"Cannot find {path_classi}. This file is generated by generate_meccano_dataset.py "
                     'with the dataset: check the selected folder.')
...
print(f"{len(nomi_ordinati)} classes read from classi.json (order preserved for fine-tuning).")
...
if not os.path.exists(js_path):
    raise SystemExit(f"Neither coco_annotations.json nor _annotations.coco.json found in {dir_train_in}")
...
if not immagini_valide:
    raise SystemExit('No images with valid annotations found. Check the dataset.')

print(f"{len(immagini_valide)} images with at least one valid annotation.")
...
print(f"Original split: {len(train_set)} train, {len(val_set)} val")
...
print(f"\nAugmentation: generating {n_da_generare} extra images to reach approximately "
      f"~{args.target} total ({n_originali_train} originals + {n_da_generare} augmented images "
      f"in train, {len(val_set)} in validation).")
...
while generate < n_da_generare and tentativi < max_tentativi:
    ...
    if generate % 200 == 0:
        print(f"  {generate}/{n_da_generare} augmented images...")

print(f"Augmented {generate} images (failed attempts: {tentativi - generate}).")
...
print(f"\nDone. train: {n_train_finale} images | validation: {n_val_finale} images")
